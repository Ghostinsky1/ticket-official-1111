...
/*
  EVENT PAGE v2 — multi-event, desktop two-column, flyer-tinted background.

  Mobile: Posh-style stack + sticky bottom CTA.
  Desktop (>=920px): screenshot layout — flyer LEFT with a glowing
  "Get Tickets from $X" pill under it, content RIGHT (title, About card,
  Location, Organizer card).

  Background: dominant colors are sampled from the flyer via canvas and
  blended into the page gradient — upload a purple flyer, the page goes
  purple. Falls back to the house rose/black if sampling fails.
*/
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      try {
        ...
        // Pick the most saturated, reasonably bright pixel bucket.
...
export default function EventPage({ eventId = DEFAULT_EVENT_ID }: { eventId?: string }) {
  ...
  const [sheet, setSheet] = useState<null | 'qty' | 'info'>(null);
  ...
  useEffect(() => { fetchQuote(ttId, qty); }, [ttId, qty, fetchQuote]);
  ...
  const ctaLabel = allSoldOut ? 'SOLD OUT' : `Get Tickets from ${money(minPrice)}`;
  const openSheet = () => !allSoldOut && setSheet('qty');
  ...
  return (
    <div style={{ minHeight: '100vh', fontFamily: F, background: `radial-gradient(130% 100% at 50% 0%, ${accent} 0%, ${deep} 55%, #000 100%)`, backgroundAttachment: 'fixed' }}>
          ...
          <div style={{ background: CARD, backdropFilter: 'blur(8px)', borderRadius: 16, padding: '18px 18px 14px', marginBottom: 28, border: '1px solid rgba(255,255,255,0.07)' }}>
            <p style={{ color: '#fff', fontSize: 15, fontWeight: 600, letterSpacing: 0.5, margin: 0 }}>
              {quote && quote.remaining <= 20 && quote.remaining > 0
                ? `ONLY ${quote.remaining} TICKETS LEFT`
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                ...
                <p style={{ color: '#b9bec8', fontSize: 14, margin: 0 }}>
                  {types.reduce((a, t) => a + (t.remaining < 0 ? 0 : 0), 0) >= 0 && quote
                    ? 'Be there' : 'Be the first in'}
                </p>
              </div>
              <button onClick={openSheet} style={{ background: 'rgba(255,255,255,0.14)', color: '#fff', border: 'none', borderRadius: 22, padding: '11px 22px', fontSize: 14.5, fontWeight: 600, cursor: 'pointer' }}>Get in</button>
            </div>
          </div>
      ...
      <div className="gz-bar">
        <button onClick={openSheet} disabled={allSoldOut}
      ...
      {/* ---------- CHECKOUT SHEET ---------- */}
      {sheet && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 50, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }} onClick={() => !submitting && setSheet(null)}>
          <div style={{ width: '100%', maxWidth: 520, background: '#0b0b0b', borderRadius: '22px 22px 0 0', padding: '22px 22px calc(26px + env(safe-area-inset-bottom))', position: 'relative' }} onClick={(e) => e.stopPropagation()}>
            <button onClick={() => !submitting && setSheet(null)} style={{ position: 'absolute', top: 18, right: 18, background: 'none', border: 'none', color: '#fff', fontSize: 20, cursor: 'pointer' }} aria-label="Close">✕</button>

            {sheet === 'qty' && (
              <>
                <p style={SHEETH}>Get tickets</p>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
                  ...
                  <div style={{ textAlign: 'center' }}>
                    <span style={{ color: '#fff', fontSize: 42, fontWeight: 800, display: 'block', lineHeight: 1 }}>{qty}</span>
                ...
                {quote && !loadingQuote && (
                  <div style={{ borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: 12, marginBottom: 18 }}>
                ...
                <button onClick={() => setSheet('info')} disabled={!quote || loadingQuote}
                  style={{ width: '100%', background: '#f2f2f2', color: '#000', border: 'none', borderRadius: 28, padding: '17px 0', fontSize: 17, fontWeight: 700, cursor: 'pointer' }}>
                  Continue
                </button>
              </>
            )}
            ...
            {sheet === 'info' && quote && (
              <>
                <p style={SHEETH}>Where do we send them?</p>
                ...
                <p style={{ color: '#6f747d', fontSize: 12.5, lineHeight: 1.5, margin: '2px 0 16px' }}>
                  {qty > 1 ? `${qty} QR codes land here — one per person.` : 'Your QR code lands here.'} Phone number is collected at payment for your text confirmation.
                </p>
                {error && <p style={{ color: ROSE, fontSize: 13.5, margin: '0 0 14px' }}>{error}</p>}
                <button onClick={pay} disabled={name.trim().length < 2 || !emailOk || submitting}
                  style={{ width: '100%', background: '#f2f2f2', color: '#000', border: 'none', borderRadius: 28, padding: '17px 0', fontSize: 17, fontWeight: 700, cursor: 'pointer', opacity: name.trim().length < 2 || !emailOk || submitting ? 0.45 : 1 }}>
                  {submitting ? 'Opening secure checkout…' : `Pay ${money(quote.total)}`}
                </button>
                <button onClick={() => setSheet('qty')} disabled={submitting} style={{ width: '100%', background: 'none', border: 'none', color: '#8a8f98', fontSize: 15, padding: '14px 0 0', cursor: 'pointer' }}>Back</button>
                <p style={{ color: '#6f747d', fontSize: 11.5, textAlign: 'center', margin: '14px 0 0' }}>Payment handled by Stripe. We never see your card.</p>
              </>
            )}
          </div>
        </div>
...
const SHEETH: React.CSSProperties = { color: '#fff', fontSize: 26, fontWeight: 800, margin: '4px 0 22px' };
